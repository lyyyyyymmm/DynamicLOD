...
S1 consists of the readiness gate, a ten-second warm-up, and a 40-second uniform orbit. S2 consists of the same readiness and warm-up phases followed by four interaction-recovery cycles. Each six-second interaction performs a cross-direction monotonic approach from 3.6 to 0.9 times the tileset bounding-sphere radius; the resulting near viewpoint is held stationary for the four-second recovery so selected refinement requests can complete. The released D-031 desktop confirmatory protocol uses S3 `pressureBurst`, which shortens each approach to four seconds, moves closer to the tileset, and extends the stationary near-view hold to six seconds. Android-A S3 is not an efficacy condition because the fixed-SSE-4 diagnostic produced no measurable tail pressure. Method order is randomized within paired blocks with seed `20260823`. Per desktop efficacy device, D1/S3 uses twelve six-method repeats and D2/S3 uses eight, yielding 120 main runs per desktop device and 240 runs across PC-A and PC-B. D1 is the primary efficacy stratum and D2 is external validation. The historical S1/S2 main queue remains audit-only and is disabled in the normal UI. All confirmatory runs use local-server LAN routing; remote LAN origins and artificial response delays are excluded from formal efficacy inference.

### 4.3 Metrics and analysis

The primary metric is the proportion of measured control windows with P95 frame time above `T`. Secondary metrics include raw-frame P95/P99, individual-frame budget violations, request peak and area under the queue curve, bytes transferred, time-weighted SSE, tile geometric error, switch rate, reversals, and recovery time. Static visual quality is calibrated at six canonical views using windowed SSIM against an SSE 4 reference; method-level formal quality evidence is predeclared for D1/D2 canonical views or timestamps linked to confirmatory blocks, with the same SSE 4 reference and a 0.02 SSIM non-inferiority margin. LPIPS is a supplementary perceptual diagnostic when its software environment is frozen.
...
**[RESULT_REQUIRED: D-031 PC-A D1/S3 confirmatory-ablation results for no-prediction, no-request, no-interaction, and no-stability variants, analyzed separately from the main six-method efficacy matrix.]**
