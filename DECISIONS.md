...
## D-031: Release frozen desktop S3 confirmatory protocol

Date: 2026-09-04

Decision: release the v2.3.6 desktop S3 confirmatory protocol for physical collection preparation, without changing the controller, workload, thresholds, baselines, datasets, frame budget, rolling window, readiness policy, or statistical plan. Formal tail-control efficacy is limited to desktop devices `pc-a` and `pc-b`. Android-A is excluded from v2.3.6/S3 efficacy inference and remains cross-device executability, telemetry-validity, and low-pressure boundary evidence only. Formal desktop collection must use a local benchmark server on each desktop (`localhost` or `127.0.0.1`), not a LAN remote origin.

Frozen matrix: D1 `bagAmsterdam` + S3 `pressureBurst` uses 12 paired repeats, and D2 `bagRotterdam` + S3 `pressureBurst` uses 8 paired repeats. Each paired block contains the six formal methods `fixed8`, `fixed16`, `cesiumDynamic`, `reactive`, `pi`, and `proposed` in deterministic seed-shuffled order using base seed `20260823`. This yields 120 runs per desktop device and 240 planned desktop confirmatory runs across `pc-a` and `pc-b`. D1 is the primary efficacy stratum; D2 is external validation and must be collected directly under the frozen route rather than after a new D2 efficacy pilot.

Implementation: the historical `buildMainExperimentQueue()` remains as a legacy S1/S2 audit queue and is marked `legacyConfirmatory=true` / `confirmatoryRelease=legacy-s1s2-unreleased`. It is disabled in the normal UI as `Legacy main batch — do not use`. The new approved UI entry is `Run frozen desktop S3 confirmatory`, backed by `buildDesktopS3ConfirmatoryQueue()` and a fail-closed preflight validator. The validator rejects wrong protocol version, unapproved device IDs including `unregistered` or `android-a`, remote server topology, non-local page origins, non-D1/D2 datasets, non-`pressureBurst` scenarios, non-formal methods including `fixedDiagnostic`, non-`lan` network profiles, non-`confirmatory` phases, mismatched release identity, and any queue that differs from the frozen D-031 plan. Manifests now record `confirmatoryRelease`, `confirmatoryRole`, and `ablationPurpose`.

Ablation is frozen separately from the main six-method inference: D1 `bagAmsterdam`, S3 `pressureBurst`, `networkProfile=lan`, `studyPhase=confirmatory-ablation`, device `pc-a`, eight repeats, seed `20260823`, and the four component variants `noPrediction`, `noRequest`, `noInteraction`, and `noStability`. These rows are not part of the main Friedman or planned Wilcoxon comparisons.

Freeze artifact: `results/confirmatory/CONFIRMATORY_FREEZE_v2.3.6.json` and `.md` record the protocol constants, device scope, server-topology rule, D1/D2 repeat counts and roles, seed, methods, statistical inputs, readiness policy, controller policy identity, PI parameters, ablation plan, full per-device queue plan, and a queue-plan SHA-256 of `8894288c1254341ccb14014f29b9d27c75a626cab7115488c8872868e96f7ad0`.

Method-level visual-quality evidence is frozen at the protocol level before formal results are interpreted: D1 and D2 use six canonical views/timestamps per method/block where feasible, reference quality is the same-view SSE 4 capture, SSIM non-inferiority margin remains 0.02, LPIPS is supplementary only, and time-weighted SSE plus visible geometric-error summaries are retained. If method-level screenshot capture is not fully implemented at collection time, C2 must remain `UNMEASURED` and no formal visual non-inferiority claim may be made from static SSE-ladder calibration alone.

Analysis isolation: formal efficacy aggregation now selects only valid D-031 confirmatory rows with `deviceId in {pc-a, pc-b}`, `dataset in {bagAmsterdam, bagRotterdam}`, `scenario=pressureBurst`, `method` in the six formal methods, `networkProfile=lan`, `serverTopology=local`, local `pageOrigin/pageHost`, and `excludeFromFormalAggregation=false`. Pilot, diagnostic, Android, remote-server, legacy S1/S2, ablation, and smoke/audit records are excluded from the main formal inference.

Evidence: failing-first tests were added for the new queue shape, deterministic method order and seeds, legacy-main preservation, fail-closed invalid contexts, D1/S3 ablation separation, D-031 analysis filtering, UI exposure, and freeze artifact hash stability. The freeze artifact was generated by `npm.cmd run lod:freeze:confirmatory`.

Reason: D-030 closed the desktop route/repeatability uncertainty, while Android-A and remote-server PC-B diagnostics showed concrete conditions that must not enter efficacy inference. D-031 converts the project from protocol exploration to frozen desktop confirmatory execution with explicit audit evidence and startup safeguards. Any substantive change after formal collection begins requires stopping the current confirmatory route, issuing a new protocol version, and reassessing affected conditions.
